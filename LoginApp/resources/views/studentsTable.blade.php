...
<head>
    ...
    <title>Students Table View</title>
...
<body class="bg-light">
    ...
    @if(session('status'))
            <div id="flash" class="alert alert-success text-center">
                    {{ session('status') }}
            </div>
    @endif
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <div class="btn-group btn-group-sm" role="group" aria-label="actions">
                                                ...
                                                {{-- Keep existing POST handlers for compatibility, but also show direct URLs below --}}
                                                <form action="{{ route('students.view') }}" method="POST" style="display:inline">
                                                    @csrf
                                                    <input type="hidden" name="id" value="{{ $s->id }}">
                                                    <button type="submit" class="btn btn-outline-info">View (form)</button>
                                                </form>

                                                <form action="{{ route('students.edit') }}" method="POST" style="display:inline">
                                                    @csrf
                                                    <input type="hidden" name="id" value="{{ $s->id }}">
                                                    <button type="submit" class="btn btn-outline-warning">Edit (form)</button>
                                                </form>
                                                ...
                                                {{-- Direct URLs for convenience (View/Edit are GETs; Delete uses a short-lived signed POST with a one-time token) --}}
                                                <div class="mt-2 text-start small text-muted">
                                                    <div>View URL: <code>{{ url('/students/view?id=' . $s['id']) }}</code></div>
                                                    <div>Edit URL: <code>{{ url('/students/edit?id=' . $s['id']) }}</code></div>

                                                    <?php
                                                        $deleteToken = \Illuminate\Support\Str::random(40);
                                                        \Illuminate\Support\Facades\Cache::put('delete_token:'.$deleteToken, ['student_id' => $s['id'], 'teacher_id' => session('teacher_id')], now()->addMinutes(5));
                                                        $signedUrl = \Illuminate\Support\Facades\URL::temporarySignedRoute('students.delete.link', now()->addMinutes(5), ['id' => $s['id'], 'teacher' => session('teacher_id')]);
                                                    ?>

                                                    <form method="POST" action="{{ $signedUrl }}" style="display:inline; margin-top:6px;">
                                                        @csrf
                                                        <input type="hidden" name="one_time_token" value="{{ $deleteToken }}">
                                                        <button type="submit" class="btn btn-sm btn-outline-danger" onclick="return confirm('Delete this student via signed link?');">Delete (signed)</button>
                                                    </form>
                                                </div>
                                            </div>
                                        </td>
